...
private class Action: UIAlertAction {
...
//        zmanimTableViewController.zmanim = ZmanimDataStore.dataSource.zmanimForTefillah(cell.tefillah!)
    }
    fileprivate lazy var selichosSelectionHandler: (Cell) -> Void = { cell in
        self.performSegue(withIdentifier: cell.segueIdentifier!, sender: cell)
    }
    fileprivate var shabbosTitle: String {
//        if let shabbos = ZmanimDataStore.dataSource.shabbos {
...
//    }
    ...
    @IBAction func changeDate(_ sender: UIBarButtonItem) {
        DatePickerDialog().show("Choose a date...", defaultDate: date, minimumDate: Date().isPastMidnightBeforeTwo ? Date().yesterday : Date(), maximumDate: Date().addingWeek(), datePickerMode: .dateAndTime) { date in
